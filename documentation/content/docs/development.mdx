...
The frontend route for admin panel is `http://your-domain.com/wp-admin/admin.php?page=gutenform`. New pages routes will be like `http://your-domain.com/wp-admin/admin.php?page=gutenform#your-page-slug`. It uses `react-router-dom` for routing which is defined in `src/admin/routes.jsx` file.
...
* Alsway use javascript `fetch` function to fetch data from backend. Use `post` method to create/update/delete data.Use `get` method only for read-only purpose. The root url for api is `gutenForm.apiUrl + 'gutenForm/v1/{custom-route}'`.
...
* If you want to pass data from the backend to the frontend as a JavaScript object, add a key-value pair in includes/Frontend/Frontend.php in the get_data method. This data will be available in the frontend as the gutenForm object or the object defined in the const OBJ_NAME variable in Frontend.php.
