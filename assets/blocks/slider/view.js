...
function tooltipFormatter(step) {
  var _step$toString$split$;
  const decimals = step >= 1 || Number.isInteger(step) ? 0 : (_step$toString$split$ = step.toString().split('.')[1]?.length) !== null && _step$toString$split$ !== void 0 ? _step$toString$split$ : 0;
  return {
    to: value => decimals === 0 ? String(Math.round(value)) : value.toFixed(decimals)
  };
}
document.addEventListener('DOMContentLoaded', () => {
  const fields = document.querySelectorAll('.gutenform-field[data-slider-min]');
  fields.forEach(field => {
    const wrapper = field.querySelector('.gutenform-slider-wrapper');
    const sliderEl = field.querySelector('.gutenform-slider');
    ...
    const start = startAttr ? JSON.parse(startAttr) : range ? [min, max] : [min];
    const orientation = field.getAttribute('data-slider-orientation') || 'horizontal';
    const direction = field.getAttribute('data-slider-direction') || 'ltr';
    const tooltips = field.getAttribute('data-slider-tooltips') === 'true';
    const connectOpt = field.getAttribute('data-slider-connect');
    const connect = range ? connectOpt !== 'false' : 'lower';
    ...
    if (paddingAttr) {
      try {
        ...
        padding = Array.isArray(parsed) ? parsed : Number(parsed);
      } catch {
        // ignore
      }
    }
    ...
    const tooltipsOption = tooltips ? range ? [formatter, formatter] : formatter : false;
    const options = {
      start,
      range: {
        min,
    ...
    nouislider__WEBPACK_IMPORTED_MODULE_0__["default"].create(sliderEl, options);
    const hiddenMin = wrapper.querySelector('.gutenform-slider-hidden-min');
    const hiddenMax = wrapper.querySelector('.gutenform-slider-hidden-max');
    const hiddenSingle = wrapper.querySelector('.gutenform-slider-hidden');
